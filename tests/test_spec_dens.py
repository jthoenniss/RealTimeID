...
class Test_spec_dens_gapless(unittest.TestCase):
    ...
    def test_spec_dens_gapless_vectorized(self):
        ...
        #above the bulk
        omegas_above_cutoff = np.linspace( self.cutoff_upper + 1, self.cutoff_upper + 100 , 10)
        #below the bulk
        omegas_below_cutoff = np.linspace(self.cutoff_lower - 100 , self.cutoff_lower - 1, 10)
        #in the bulk
        omegas_between_cutoffs = np.linspace(self.cutoff_lower + 1, self.cutoff_upper - 1, 10)
    ...
    def test_spec_dens_scalar(self):
        ...
        self.assertTrue(np.isclose(self.spec_dens(self.cutoff_lower + (self.cutoff_upper - self.cutoff_lower) / 2 ), self.Gamma, rtol=1e-10))
        #below the bulk
        self.assertTrue(np.isclose(self.spec_dens(self.cutoff_lower - 1), 0., rtol=1e-10))
        #above the bulk
        self.assertTrue(np.isclose(self.spec_dens(self.cutoff_upper + 1), 0., rtol=1e-10))
...
class Test_spec_dens_gapped(unittest.TestCase):
    ...
    def setUp(self):
        # define test parameters
        self.cutoff_lower = 2.#lower cutoff frequency for positive frequencies (defines upper cutoff of the gap)
        self.cutoff_upper = 10. #upper cutoff frequency for positive frequencies
        self.Gamma = 1.0
    ...
    def test_spec_dens_gapped_vectorized(self):
        ...
        omegas_above_upper_bulk = np.linspace( self.cutoff_upper + 1, self.cutoff_upper + 100 , 10)
        #in the upper bulk
        omegas_in_upper_bulk = np.linspace( self.cutoff_lower + 1 , self.cutoff_upper - 1, 10)
        #in the gap (below upper bulk and above lower bulk)
        omegas_in_gap = np.linspace(- self.cutoff_lower + 1, self.cutoff_lower - 1, 10)
        #in the lower bulk
        omegas_in_lower_bulk = np.linspace( -self.cutoff_upper + 1 , -self.cutoff_lower - 1, 10)
        #below the lower bulk
        omegas_below_lower_bulk = np.linspace( -self.cutoff_upper -100 , -self.cutoff_upper - 1 , 10)
        
        #_________Test implementation of spectral density_________  
        #check if spectral density is zero above upper bulk
        self.assertTrue(np.all(np.isclose(self.spec_dens(omegas_above_upper_bulk), 0. , rtol=1e-10)))
        #check if spectral density is Gamma below upper bulk
        self.assertTrue(np.all(np.isclose(self.spec_dens(omegas_in_upper_bulk), self.Gamma , rtol=1e-10)))
        #check if spectral density is zero in gap
        self.assertTrue(np.all(np.isclose(self.spec_dens(omegas_in_gap), 0. , rtol=1e-10)))
        #check is spectral density is Gamma in lower bulk
        self.assertTrue(np.all(np.isclose(self.spec_dens(omegas_in_lower_bulk), self.Gamma , rtol=1e-10)))
        #check if spectral density is zero below lower bulk
        self.assertTrue(np.all(np.isclose(self.spec_dens(omegas_below_lower_bulk), 0. , rtol=1e-10)))
